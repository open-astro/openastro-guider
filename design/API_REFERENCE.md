...
| `set_guider_options` | `FastRecenter`?, `AutoSelDownsample`? (0–4), `TolerateJumps`?, `TolerateJumpsThreshold`? | **(Batch C)** Set the niche guider options (all optional). `TolerateJumps*` require the multi-star guider. |
