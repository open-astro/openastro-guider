...
### Removed
- A `#if defined(_WINDOWS)` block in `phd.h` that pulled in `winsock2.h`/`windows.h` — missed by the earlier inline-ifdef strip because it used the `_WINDOWS` spelling (not `_WIN32`/`__WINDOWS__`). `src/` now contains **zero** platform `#if`s of any spelling, and no `winsock`/`windows.h`/`NOMINMAX` references.
